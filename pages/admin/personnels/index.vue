<template>
  <NuxtLayout name="admin" page-title="ข้อมูลบุคลากร" page-description="ภาพรวมข้อมูลบุคลากรทั้งหมดในโรงเรียน">
...
const fetchItems = async () => {
  loading.value = true
  try {
    const response = await apiFetch<any>('/staffs', { params: { page: pagination.value.page, limit: pagination.value.limit } })
...
const handleView = (row: any) => {
  if (!row?.id) return
  navigateTo(`/admin/personnels/${row.id}`)
}
