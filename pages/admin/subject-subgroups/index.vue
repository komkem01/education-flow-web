<template>
  <NuxtLayout name="admin" page-title="กลุ่มย่อยวิชา" page-description="จัดการกลุ่มย่อยของวิชาในระบบ">
...
const columns = [
  { key: 'name', label: 'ชื่อกลุ่มย่อยวิชา' },
  { key: 'code', label: 'รหัสกลุ่มย่อยวิชา' },
...
const fetchItems = async () => {
  loading.value = true
  try {
    const response = await apiFetch<any>('/subject-subgroups', { params: { page: pagination.value.page, limit: pagination.value.limit } })
    ...
    error('ไม่สามารถโหลดข้อมูลกลุ่มย่อยวิชาได้')
  } finally {
    loading.value = false
  }
}
